...
class Summit:
    def __init__(self, e1, e2, e3, e4, e5, e5_1, e6, e6_1, e7, e8, e8_1, e9, v_1, v_2, root, list_name):
        ...
        self.v_2 = v_2
        self.root = root
        self.pdf = canvas.Canvas(f"{self.entry_3.get()}{self.datetime}.pdf")
        ...
        try:
            ...
            for name, qui, rate, size ,list in self.list_name:
    ...
    def prooses(self):
        ...
        num = 1
        totel = 0
        for list, qty, rate, size,list_sq in self.list_name:
            string =""
            for ls in list_sq[1:]:
                string+=f" {ls} "
            ...
            if len(list) > 45:
                ...
                self.pdf.drawString(372, y, f"{qty} {self.v_1.get()}{self.v_2.get()}")
                self.pdf.drawString(427, y, f"{rate}/-{string}")
                self.pdf.drawString(482, y, f"{str('%.2f' %amount)}/-")
        ...
        self.pdf.drawString(482, 190, f"{str('%.2f' %totel)}/-")

        gst_va = totel * (self.gst / 100)
        all_totel = int(round(totel + gst_va))

        all_totel_str = str("%.2f" % all_totel)
        print("%.2f" % gst_va)
        gst_va_str = str("%.2f" % gst_va)
        ...
        if self.papa == 2:
            calculation.clculation.replace("self", self.ent_1)
            ...
            try :
                ...
                subprocess.Popen(f"{self.ent_3}{self.datetime}.pdf", shell=True)
            except:
                return
        else:
            return
    ...
    def add(self, gst):
        gst_va_haff = gst / 2
        gst_va_haff_str = str("%.2f" % gst_va_haff)
